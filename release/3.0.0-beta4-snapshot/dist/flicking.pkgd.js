    ...
    */
    ...
    function () {
      ...
      __proto.change = function (input, event, offset) {
        ...
        console.log(depaPos.flick, destPos.flick);
    ...
    **/
    ...
    function (_super) {
      ...
      __proto.onRelease = function (e, context) {
        ...
        var inputDelta = Math.abs(horizontal ? inputEvent.deltaX : inputEvent.deltaY);
        var isNextDirection = Math.abs(velocity) > 0 ? velocity < 0 : delta > 0;
        var swipeDistance = Math.max(Math.abs(delta), inputDelta);
        ...
        if (overThreshold) {
          ...
          } else if (!isFreeScroll && !viewport.isOutOfBound() && (swipeDistance <= minimumDistanceToChange // It's the same if there's not enough distance.
        ...
        var eventType = !overThreshold || movingToSamePanel ? isFreeScroll ? EVENTS.NONE : EVENTS.RESTORE : EVENTS.CHANGE;
      ...
      __proto.findRestorePanelInCircularMode = function (isNextDirection, viewport) {
        ...
        var panelToMove = !isNextDirection && lapped ? firstClonedPanel : currentPanel;
        return panelToMove;
      };
    ...
    function () {
      ...
      __proto.moveTo = function (panel, eventType, axesEvent, offset, duration) {
        ...
        eventResult.onSuccess(function () {
          ...
          currentState.direction = estimatedPosition > currentPosition ? DIRECTION.NEXT : DIRECTION.PREV; // freeScroll only occurs in release events
          ...
          if (axesEvent && axesEvent.setTo) {
            ...
            if (estimatedPosition === currentPosition) {
              // no move
              _this.nearestPanel = _this.findNearestPanel();
            } else {
              _this.axes.setTo({
      ...
      __proto.findNearestPanel = function () {
        ...
        if (this.isOutOfBound()) {
          ...
          if (position < prevRange) {
            ...
            var currentAnchorPosition = currentPanel.getAnchorPosition();

            if (position === prevRange && currentAnchorPosition <= prevRange + relativeHangerPosition || position === nextRange && currentAnchorPosition >= nextRange + relativeHangerPosition) {
              return currentPanel;
            }
          }
        }
      ...
      __proto.updateOriginalPanelPositions = function () {
        ...
        var currentPanel = this.currentPanel;
        var nearestPanel = this.nearestPanel;
        var currentState = this.stateMachine.getState(); // Update panel position && fit to wrapper
        ...
        if ((currentState.holding || currentState.playing) && nearestPanel) {
          ...
          var currentIndex = currentPanel.getIndex() + (currentPanel.getCloneIndex() + 1) * panels.length;
          var nearestIndex = nearestPanel.getIndex() + (nearestPanel.getCloneIndex() + 1) * panels.length;
          var looped = currentIndex >= nearestIndex && currentState.delta > 0 || currentIndex <= nearestIndex && currentState.delta < 0;
    ...
    function (_super) {
      ...
      __proto.moveTo = function (index, duration) {
        ...
        var eventType = panel.getIndex() === viewport.getCurrentIndex() ? EVENTS.NONE : EVENTS.CHANGE;
      ...
      Flicking.VERSION = "3.0.0-beta4-snapshot";
      /**
