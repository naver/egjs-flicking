...
describe("Viewport", () => {
  describe("Properties", () => {
    ...
    it("should have property element same to the given element", async () => {
      ...
      expect(viewport.element).toBe(expected);
    });
    ...
    it("has size 0 until updating it", async () => {
      const flicking = await createFlicking(El.DEFAULT_HORIZONTAL);
      const viewportEl = El.viewport().appendTo(container).setHeight(300).el;

      const viewport = new Viewport(flicking, viewportEl);

      expect(viewportEl.clientWidth).not.toBe(0);
      expect(viewportEl.clientHeight).not.toBe(0);
    ...
    it("should set size equal to the given element after update", async () => {
      ...
      const flicking = await createFlicking(El.DEFAULT_HORIZONTAL);
      const viewportEl = El.viewport().appendTo(container).setWidth(expected.width).setHeight(expected.height).el;
    ...
    it("can set its size equal to the given arguments", async () => {
      ...
      const viewportEl = El.viewport().appendTo(container).setWidth(previous.width).setHeight(previous.height).el;
  ...
  describe("Methods", () => {
    ...
    describe("resize", () => {
      it("should set width/height to viewport element's widht/height without CSS border", async () => {
        const flicking = await createFlicking(El.DEFAULT_HORIZONTAL);
        const viewportEl = El.viewport().setWidth("1000px").setHeight("500px").appendTo(container).el;
        const viewport = new Viewport(flicking, viewportEl);
