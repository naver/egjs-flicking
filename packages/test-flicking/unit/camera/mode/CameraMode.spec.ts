...
describe("CameraMode", () => {
  ...
  describe("getAnchors", async () => {
    it("should return anchors matching panels without considering reachability", async () => {
      const flicking = await createFlicking(
        El.viewport()
          .setWidth(900)
          .add(El.camera().add(El.panel("100px")).add(El.panel("100px")).add(El.panel("100px"))),
        { bound: true }
      );
      ...
      expect(anchors.length).toBe(panels.length);
      expect(anchors.every((anchor, idx) => anchor.panel === panels[idx])).toBe(true);
    });
  });
  ...
  describe("clampToReachablePosition", () => {
    it("should return position if it's inside current range", async () => {
      ...
      expect(mode.clampToReachablePosition(range.min)).toBe(range.min);
      expect(mode.clampToReachablePosition((range.min + range.max) / 2)).toBe((range.min + range.max) / 2);
      expect(mode.clampToReachablePosition(range.max)).toBe(range.max);
    });
    ...
    it("should return clamped position if it's outside of current range", async () => {
      ...
      expect(mode.clampToReachablePosition(range.min - 1)).toBe(range.min);
      expect(mode.clampToReachablePosition(range.max + 1)).toBe(range.max);
    });
  });
  ...
  describe("canSee", () => {
    it("should return true when panel is inside of camera's visible range", async () => {
      ...
      vi.spyOn(panel, "position" as any, "get").mockReturnValue(camera.visibleRange.min);

      expect(mode.canSee(panel)).toBe(true);

      vi.spyOn(panel, "position" as any, "get").mockReturnValue(camera.visibleRange.max);
    ...
    it("should return false when panel is outside of camera's visible range", async () => {
      ...
      vi.spyOn(panel, "range" as any, "get").mockReturnValue({
        min: camera.visibleRange.min - 1,
        max: camera.visibleRange.min - 1
      });

      expect(mode.canSee(panel)).toBe(false);

      vi.spyOn(panel, "range" as any, "get").mockReturnValue({
        min: camera.visibleRange.max + 1,
        max: camera.visibleRange.max + 1
      });
  ...
  describe("canReach", () => {
    it("should return false when panel's position is outside of camera range", async () => {
      ...
      expect(mode.canReach(panel)).toBe(false);
    ...
    it("should return true when panel's position is inside of camera range", async () => {
      ...
      vi.spyOn(panel, "position" as any, "get").mockReturnValue(camera.range.max);

      expect(mode.canReach(panel)).toBe(true);

      vi.spyOn(panel, "position" as any, "get").mockReturnValue(camera.range.min);
  ...
  describe("getCircularOffset", () => {
    it("should return 0", async () => {
      ...
      expect(mode.getCircularOffset()).toBe(0);
    });
  });
});
