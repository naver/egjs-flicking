import { DiffResult, diff } from "@egjs/list-differ";
...
describe("sync", () => {
  it("can remove consecutive panels", async () => {
    ...
    expect(flicking.panels.length).toBe(1);
    expect(flicking.panels[0]).toEqual(prevPanels[0]);
  });
  ...
  it("can remove the first panel", async () => {
    ...
    expect(flicking.panels.length).toBe(2);
    expect(flicking.panels).toEqual(prevPanels.slice(1, 3));
  });
  ...
  it("can remove the last panel", async () => {
    ...
    expect(flicking.panels.length).toBe(2);
    expect(flicking.panels).toEqual(prevPanels.slice(0, 2));
  });
  ...
  it("can remove non-consecutive panels", async () => {
    ...
    expect(flicking.panels.length).toBe(1);
    expect(flicking.panels).toEqual([prevPanels[1]]);
  });
  ...
  it("can remove consecutive & non-consecutive mixed panels", async () => {
    ...
    expect(flicking.panels.length).toBe(nonRemovedPanels.length);
    expect(flicking.panels).toEqual(nonRemovedPanels);
  });
  ...
  it("should decrease the indexes of the middle panels if one of the panels move to last", async () => {
    ...
    expect(flicking.panels.every((panel, idx) => panel.index === idx)).toBe(true);
  });
  ...
  it("should bind new elements to new panels correctly when previous panels are all removed", async () => {
    ...
    expect(newPanels.every((panel, idx) => panel.element === newElements[idx])).toBe(true);
  });
});
