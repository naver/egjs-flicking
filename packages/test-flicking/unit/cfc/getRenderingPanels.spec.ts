...
describe("getRenderingPanels", () => {
  it("should use diffResult's list instead of prevList", async () => {
    ...
    expect(renderingPanels.every((panel, idx) => panel !== prevPanels[idx])).toBe(true);
    expect(renderingPanels.every((panel, idx) => panel === newPanels[idx])).toBe(true);
  });
  ...
  it("should not include removed panels", async () => {
    ...
    expect(renderingPanels.every(panel => !removed.includes(panel))).toBe(true);
  });
  ...
  it("should include added panels after the previous panels", async () => {
    ...
    expect(renderingPanels.slice(0, prevPanels.length)).toEqual(prevPanels);
    expect(renderingPanels.slice(prevPanels.length)).toEqual(added);
  });
});
