...
describe("VirtualManager", () => {
  describe("Options", () => {
    describe("renderPanel", () => {
      it("should set visible panel element's innerHTML by its result", async () => {
        ...
        expect(flicking.visiblePanels.length).not.toBe(0);
        expect(flicking.visiblePanels.every(panel => panel.element.innerHTML === `Panel ${panel.index}`)).toBe(true);
      });
    });
    ...
    describe("initialPanelCount", () => {
      it("should create the same number of panels as it", async () => {
        ...
        expect(flicking.panels.length).toBe(panelCount);
      });
    });
    ...
    describe("panelClass", () => {
      it("should add `panelClass` to the collected elements", async () => {
        ...
        expect(virtual.elements.every(virtualEl => virtualEl.nativeElement.classList.contains(expectedClass))).toBe(
          true
        );
      });
    });
    ...
    describe("cache", () => {
      it("should use panel's cached innerHTML instead of calling it again", async () => {
        const renderStub = vi.fn().mockReturnValue("SOME_INNERHTML");
        const flicking = await createFlicking(El.DEFAULT_HORIZONTAL_WITH_PANELS(0), {
          panelsPerView: 1,
          virtual: {
        ...
        const prevRenderCallCount = renderStub.mock.calls.length;

        // Render again
        await renderer.forceRenderAllPanels();
        await renderer.render();

        expect(renderStub).toHaveBeenCalledTimes(prevRenderCallCount);
      });
    });
  });
  ...
  describe("Methods", () => {
    describe("render", () => {
      it("should set invisible panel's display to none", async () => {
        ...
        const invisibles = flicking.virtual.elements.filter(virtualEl =>
          visibles.every(visible => visible.element !== virtualEl.nativeElement)
        );

        expect(visibles).not.toHaveLength(0);
  ...
  describe("append", () => {
    it("should create panels by count of it", async () => {
      ...
      expect(flicking.panels.length).toBe(200);
    });
    ...
    it("should not increase previous panel's indexes", async () => {
      ...
      expect(prevPanels.every((panel, index) => panel.index === index)).toBe(true);
    });
  });
  ...
  describe("prepend", () => {
    ...
    it("should increase previous panel's indexes", async () => {
      ...
      expect(prevPanels.every((panel, index) => panel.index === index + 100)).toBe(true);
    });
  });
  ...
  describe("insert", () => {
    ...
    it("should not increase panel's indexes before it, but only the panels after it", async () => {
      ...
      expect(prevPanels.slice(0, 50).every((panel, index) => panel.index === index)).toBe(true);
      expect(prevPanels.slice(50).every((panel, index) => panel.index === index + 50 + 100)).toBe(true);
    });
  });
  ...
  describe("remove", () => {
    it("should remove panels by count of it", async () => {
      ...
      expect(flicking.panels.length).toBe(50);
    });
    ...
    it("should not decrease panel's indexes before it, but only the panels after it", async () => {
      ...
      expect(prevPanels.slice(0, 50).every((panel, index) => panel.index === index)).toBe(true);
      expect(prevPanels.slice(51).every((panel, index) => panel.index === index + 51 - 1)).toBe(true);
    });
  });
});
