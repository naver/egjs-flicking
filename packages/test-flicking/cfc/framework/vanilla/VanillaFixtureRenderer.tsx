...
import { cleanup, createSandbox, findFlickingJSX, resolveFlickingWhenReady } from "../../common/utils";
...
const render = async (el: JSX.Element): Promise<Flicking> => {
  ...
  const html = renderToStaticMarkup(parsedEl as React.ReactElement);
...
const parseJSX = (el: JSX.Element): JSX.Element => {
  ...
  if (!isValidElement(el)) {
    ...
    const cameraEl = createElement<Partial<HTMLElement>>(
      cameraTag,
      {
    ...
    const viewportEl = createElement<Partial<HTMLElement>>(
      tag,
      {
