...
- 브랜치 `test/e2e-{category}-{id}` → PR 제목 `test(e2e): {category}/{id} 케이스 보강`. **1 PR = 1 케이스.** (`pagination`, `parallax`처럼 카테고리가 다른 동명 케이스가 있어 브랜치명에 카테고리를 포함한다)
- 작업 범위는 스펙(YAML) + 테스트. 
...
- **처음 맡는 사람은 샘플 PR [#959](https://github.com/naver/egjs-flicking/pull/959)를 먼저 읽는다.** `basic/threshold` 케이스를 실제로 보강한 PR이며, 레퍼런스로 계속 열어 둔다.
