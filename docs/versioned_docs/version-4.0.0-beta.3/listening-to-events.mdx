...
In addition, we're providing one additional event to Angular.
`renderPanelChange` event is an Angular-exclusive event that can be used for [renderOnlyVisible](api/Flicking#renderOnlyVisible) event.
```html
<ngx-flicking
