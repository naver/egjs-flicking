...
/**
 * @interface
 * @property {$ts:Flicking.align}
 * @property - Index of the panel to set as default when initializing. A zero-based integer.<ko>{@link Flicking#init Flicking.init()} 호출시 이동할 디폴트 패널의 인덱스로, 0부터 시작하는 정수입니다.</ko>
 * @param {"prev" | "center" | "next" | string | number | object} [options.align="center"] The reference position of the hanger in the viewport, which hangs panel anchors should be stopped at.<br/>It should be provided in px or % value of viewport size.<br/>You can combinate those values with plus/minus sign.<br/>ex) "50", "100px", "0%", "25% + 100px"<ko>뷰포트 내부의 행어의 위치. 패널의 앵커들이 뷰포트 내에서 멈추는 지점에 해당한다.<br/>px값이나, 뷰포트의 크기 대비 %값을 사용할 수 있고, 이를 + 혹은 - 기호로 연계하여 사용할 수도 있다.<br/>예) "50", "100px", "0%", "25% + 100px"</ko>
 * @param {string | number} [options.align.panel]
 * @param {string | number} [options.align.camera]
...
class Flicking extends Component<FlickingEvents> {
   ...
   * @type {"prev" | "center" | "next" | string | number | { panel: string | number, camera: string | number }}
   * @property {string | number} panel <ko>패널에 적용할 값</ko>
   * @property {string | number} camera
   * @default "center"
   * @example
   * ```ts
   ...
   * @param options An option object for Flicking.<ko>Flicking에 적용할 옵션 오브젝트</ko>
